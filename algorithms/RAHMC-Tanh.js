...
MCMC.registerAlgorithm("RAHMC-Tanh", {
  description: "RA-HMC with Tanh Friction Schedule",
  ...
  attachUI: (self, folder) => {
    ...
    folder.add(self, "steepness", 1.0, 10.0).step(0.5).name("Steepness");
    folder.open();
  },
  ...
  step: (self, visualizer) => {
    ...
    // conformal leapfrog integration with tanh-scheduled friction
    const q = q0.copy();
    const p = p0.copy();
    ...
    // tanh friction schedule: gamma(t) = gamma_max * tanh(steepness * (2*t/T - 1))
    for (let step = 0; step < self.leapfrogSteps; step++) {
      const currentTime = step * self.dt;
      const normalizedTime = self.steepness * (2.0 * currentTime / totalTime - 1.0);
      const gamma_t = self.gamma * Math.tanh(normalizedTime);
